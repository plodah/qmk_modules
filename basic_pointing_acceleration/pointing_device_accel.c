...
report_mouse_t pointing_device_task_pointing_device_accel(report_mouse_t mouse_report) {
    ...
    float tempacc = (g / 2)*(velocity - s) + m;
    if(tempacc < m){const float pointing_device_accel_factor = m;}
    else if(tempacc > 1){const float pointing_device_accel_factor = 1;}
    else{const float pointing_device_accel_factor = tempacc;}
    ...
    pd_dprintf("PDACCEL: DPI:%4i Grw: %.3f Ofs: %.3f Lmt: %.3f | Fct: %.3f v.in: %.3f v.out: %.3f d.in: %3i "
               "d.out: %3i\n",
               device_cpi, g_pointing_device_accel_config.growth_rate,
