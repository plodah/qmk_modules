...
_Static_assert(sizeof(g_basic_pointing_acceleration_config) <= VIA_EEPROM_CUSTOM_CONFIG_SIZE,
...
void pointing_device_config_set_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_pointing_device_takeoff:
            {
                ...
                g_basic_pointing_acceleration_config.takeoff =
        ...
        case id_pointing_device_limit:
            {
                ...
                g_basic_pointing_acceleration_config.limit = PROJECT_FROM_VIA(limit, MACCEL_VIA_LMT_MIN, MACCEL_VIA_LMT_MAX);
#ifdef MACCEL_DEBUG
                printf("MACCEL:via: tko: %.3f grw: %.3f ofs: %.3f LMT: %.3f\n", g_basic_pointing_acceleration_config.takeoff,
                       g_basic_pointing_acceleration_config.growth_rate, g_basic_pointing_acceleration_config.offset,
                       g_basic_pointing_acceleration_config.limit);
        ...
        case id_pointing_device_enabled:
            {
                g_basic_pointing_acceleration_config.enabled = value_data[0];
                break;
            }
    }
}
...
void pointing_device_config_get_value(uint8_t *data) {
    ...
    switch (*value_id) {
        case id_pointing_device_takeoff:
            {
                // uint16_t takeoff = (g_basic_pointing_acceleration_config.takeoff - 0.5) / (4.5 / 6) * 10000;
                uint16_t takeoff =
                    PROJECT_TO_VIA(g_basic_pointing_acceleration_config.takeoff, MACCEL_VIA_TKO_MIN, MACCEL_VIA_TKO_MAX);
        ...
        case id_pointing_device_growth_rate:
            {
                uint16_t growth_rate =
                    PROJECT_TO_VIA(g_basic_pointing_acceleration_config.growth_rate, MACCEL_VIA_GRO_MIN, MACCEL_VIA_GRO_MAX);
        ...
        case id_pointing_device_offset:
            {
                uint16_t offset =
                    PROJECT_TO_VIA(g_basic_pointing_acceleration_config.offset, MACCEL_VIA_OFS_MIN, MACCEL_VIA_OFS_MAX);
        ...
        case id_pointing_device_limit:
            {
                uint16_t limit =
                    PROJECT_TO_VIA(g_basic_pointing_acceleration_config.limit, MACCEL_VIA_LMT_MIN, MACCEL_VIA_LMT_MAX);
        ...
        case id_pointing_device_enabled:
            {
                value_data[0] = g_basic_pointing_acceleration_config.enabled;
                break;
            }
    }
}
...
__attribute__((weak)) void pointing_device_config_update(basic_pointing_acceleration_config_t *config) {
    via_update_custom_config(config, 0, sizeof(basic_pointing_acceleration_config_t));
}

__attribute__((weak)) void pointing_device_config_read(basic_pointing_acceleration_config_t *config) {
    via_read_custom_config(&g_basic_pointing_acceleration_config, 0, sizeof(g_basic_pointing_acceleration_config));
}
